	...
	ifstream file(file_to_search_in, ios::in | ios::binary);
		if(!file.is_open()){
			...
			if(found_pos != string::npos){
				...
				//now since we got the pattern we are searching we can stop the search by killing all the other and all the other processes
				//now we just need to call kill(0)
				kill(0, SIGKILL);
			}
			else{
